...
type Props = {
  data: NonNullable<EVENTS_PAGE_QUERY_RESULT>['reviewsSection']
  locale: Locale
}

// Reużywa ReviewsCarousel (jak homepage / hotel). Carousel z mocka
// `getGoogleReviews()` — w F8 podmienimy na prawdziwe Google Places API.
export async function EventsReviews({ data, locale }: Props) {
  if (!data) return null
      ...
      <div className="layout-container flex flex-col gap-10 md:gap-[54px]">
        <Reveal>
          <header className="mx-auto flex max-w-[874px] flex-col items-center gap-3 text-center md:gap-4">
            ...
            {title && (
              <h2 className="text-text text-4xl leading-none font-normal tracking-tight md:text-5xl md:tracking-[-0.03em] lg:text-6xl">
                {title}
              </h2>
            )}
