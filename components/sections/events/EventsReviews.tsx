...
// F8 podepnie real Google Places API (lib/googleReviews.ts).
// Na razie statyczna placeholder-karta-CTA, identyczna z homepage ReviewsBlock.
export function EventsReviews({ data, locale }: Props) {
  if (!data) return null
    ...
    <section className="bg-bg py-20 md:py-32">
      <div className="layout-container flex flex-col gap-12">
        <header className="flex flex-col items-center gap-4 text-center">
          ...
          {title && (
            <h2 className="text-text max-w-3xl text-4xl font-light tracking-tight md:text-5xl md:tracking-[-0.03em] lg:text-6xl">
              {title}
            </h2>
          )}
